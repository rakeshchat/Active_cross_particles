...
//L(100),trelax(10000),tmax(100000),realz(10),(rho=0.13) = 10'
...
int      a[L][L],a_prime[L][L],position1[L*L]={0},position2[L*L]={0},NN,y,z,i,j,p,q,e,gap=50000;
long int t,tt,Tr,Tm,x1,x2,rotation=0;
double   rho,c,d,d1,slot1,slot2,slot3,slot4,block1,block2,rT=1.0,rA,rR,r,dt,pT,pA,pR,pTA,u,v,y1,y2,slope;
double   *s,*xt,*yt,*xtsd,*ytsd,*msd,*msdav;
char     outputfile[200];
...
//=============================================
sprintf(outputfile, "msd%d-%.2f-%.2f",(int)rA,rR,rho);
//sprintf(outputfile, "msdrA10-%.2f-%.2fdata%.f",rR,rho,seconds);
...
for(t=0;t<Tm;t++){s[t]=0; xt[t]=0; yt[t]=0; xtsd[t]=0; ytsd[t]=0; msd[t]=0; msdav[t]=0;}
...
//choosing active particle for rotational move
i = 0;
p = position1[i]; q = position2[i];
...
p = position1[i]; q = position2[i];
  ...
  for(t=0;t<Tm;t++){msdav[t] = xtsd[t] + ytsd[t] ;}
